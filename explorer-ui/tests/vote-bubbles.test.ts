...
describe('radiusScale', () => {
  ...
  it('never exceeds half the canvas height', () => {
    expect(radiusScale([1], 1, COLUMN)).toBeLessThanOrEqual(300 / 2.6)
  })

  it('keeps a dust vote visible instead of collapsing it to nothing', () => {
    const scale = radiusScale([1e21, 1], 1e21, COLUMN)
    const dustRadius = Math.max(2.5, Math.sqrt(1 / 1e21) * scale)

    expect(dustRadius).toBe(2.5)
  })
  ...
  it('fills roughly the intended fraction of the column', () => {
    ...
    // 0.42 of the column, the packing factor circles can actually reach.
    expect(area / (COLUMN * 300)).toBeCloseTo(0.42, 2)
  })

  it('degrades safely on empty or zero input', () => {
    expect(radiusScale([], 0, COLUMN)).toBe(2.5)
    expect(radiusScale([0, 0], 0, COLUMN)).toBe(2.5)
  })
})
