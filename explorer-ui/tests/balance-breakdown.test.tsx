...
describe('BalanceBreakdown', () => {
  it('renders nothing when the balance has no locked or reserved part', () => {
    const html = renderToStaticMarkup(<BalanceBreakdown balance={bal({ reserved: '0', frozen: undefined, breakdown: undefined })} />)
    expect(html).toBe('')
  })

  it('one bar + schedule sorted by time to liquidity, transferable always first', () => {
    const html = renderToStaticMarkup(<BalanceBreakdown balance={bal({
      frozen: raw(500),
      breakdown: [
    ...
    // transferable is the first schedule row
    expect(html).toContain('transferable')
    ...
    // order: transferable → releasable → clearable statics → still-cast votes
    // → hard dated locks ascending
    const order = ['transferable', 'unstake to free', 'until pulled', 'until cleared', 'while voting/delegating', 'if unstaked now']
