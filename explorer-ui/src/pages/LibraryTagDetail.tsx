import { useLibraryTag, useLibraryTagListCount, useLibraryTagValueEvents } from '../hooks/useUser'
  ...
  return (
    <div className="wrap">
      <div className="page-head">
        <Crumbs items={[{ label: 'Home', to: paths.dashboard() }, { label: 'Libraries', to: paths.libraries() }, { label: data?.name ?? tagId }]} />
      </div>
              ...
              <div className="acct-head">
                ...
                <div className="acct-meta">
                  ...
                  {/* `.full` is shared with the address page's break-all address line;
                      this page's content is prose ("N accounts · view library"), so it
                      opts out of that per-character wrap and wraps at word boundaries
                      instead — otherwise "accounts"/"library" mangle mid-word on a
                      narrow viewport once both phrases no longer fit on one line. */}
                  <div className="full" style={{ wordBreak: 'normal', flexWrap: 'wrap' }}>
                    <span className="muted">{members.length} accounts</span> · <Link to={paths.library(libraryId)}>view library</Link>
                  </div>
                </div>
