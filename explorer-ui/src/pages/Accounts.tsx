...
export function Accounts() {
  ...
  // A viewer's own tags fold server-side now, exactly like system tags (see
  // useAccounts) — every row here, system or viewer tag, already arrives as
  // one group row with group values, so rendering is the same loop either way.
  const displayRows = rows.map((r, i) => <AccountRow key={rowKey(r, i)} r={r} />)
