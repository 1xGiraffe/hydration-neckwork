...
- Every query on the server runs under a wall-clock ceiling, declared in `clickhouse/users.d/profile-limits.xml` on the `default` profile all services connect through: `max_execution_time = 600` with `timeout_overflow_mode = throw` (never `break`, which returns a truncated result as if complete), constrained to `min 1` / `max 14400` so a client can neither switch the limit off (`0` is refused with `SETTING_CONSTRAINT_VIOLATION`) nor lift it past four hours. The numbers come from `system.query_log`, not taste: over 14 days the slowest statement that ran without a setting of its own was a 152 s ad-hoc scan, ingestion never exceeded 3.8 s and the backups 73 ms, so the default reaches only an abandoned query — the case it exists for is `count()` over `system.numbers` with a `LIMIT` (the limit bounds the one result row, not the input, so it never finishes), left behind because a Bash-tool timeout does not stop a `docker exec … clickhouse-client` child. Clients with a reason to differ say so on the connection: `api`/`api-public`/`api-data` run every read at 20 s (`API_CLICKHOUSE_SETTINGS`, `request_timeout` 25 s), `derivations` and the bootstrap/queue drains at 3600 s (`createLongOpClickHouseClient`), account affinity at 5 s; `raw-live`/`main-live`, the snapshot loops, the backups and `schema-bootstrap` take the default, and async-insert flushes inherit the originating insert's value. A sanctioned longer ad-hoc pass passes `--max_execution_time=N` explicitly. The external read-only readers keep their own `readonly_analyst` profile (120 s, CONST; managed outside the repo). A `users.d` change applies without a restart — `docker cp` it into `/etc/clickhouse-server/users.d/`, `SYSTEM RELOAD CONFIG` (the only step that reports a parse error; an XML comment containing `--` is one), verify on a fresh connection with `SELECT name, value, min, max FROM system.settings WHERE name = 'max_execution_time'` — and `docker compose build clickhouse` so the image carries it to the next recreate.
