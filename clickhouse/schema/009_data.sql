...
CREATE TABLE IF NOT EXISTS price_data.intent_orders_by_account (`owner` String, `block_height` UInt32, `event_index` UInt32, `intent_id` UInt128, `seq` UInt64, `kind` LowCardinality(String), `asset_in` UInt32, `asset_out` UInt32, `amount_in` String, `amount_out` String, `partial` UInt8, `partial_min` String, `slippage_ppm` UInt32, `budget` String, `period` UInt32, `deadline_ms` UInt64, `forward_contract` String, `block_timestamp` DateTime, `extrinsic_index` Nullable(UInt32), `ingested_at` DateTime) ENGINE = ReplacingMergeTree(ingested_at) ORDER BY (owner, block_height, event_index) SETTINGS index_granularity = 8192;
CREATE MATERIALIZED VIEW IF NOT EXISTS price_data.intent_orders_by_account_mv TO price_data.intent_orders_by_account (`owner` String, `block_height` UInt32, `event_index` UInt32, `intent_id` UInt128, `seq` UInt64, `kind` LowCardinality(String), `asset_in` UInt32, `asset_out` UInt32, `amount_in` String, `amount_out` String, `partial` UInt8, `partial_min` String, `slippage_ppm` UInt32, `budget` String, `period` UInt32, `deadline_ms` UInt64, `forward_contract` String, `block_timestamp` DateTime, `extrinsic_index` Nullable(UInt32), `ingested_at` DateTime) AS SELECT owner, block_height, event_index, intent_id, seq, kind, asset_in, asset_out, amount_in, amount_out, partial, partial_min, slippage_ppm, budget, period, deadline_ms, forward_contract, block_timestamp, extrinsic_index, ingested_at FROM price_data.intent_orders WHERE owner != '';
