...
const DESCRIPTION = [
  ...
  '**periodAggregate** is the sum of the returned buckets, except on `hsm_revenue`, where the incumbent returns their MEAN and this reproduces it.',
  ...
  '**`uniswap_v3_fee`** (product `uniswap-v3`, not in the incumbent\'s matrix) is the protocol\'s take from the concentrated-liquidity (Uniswap v3) pools on Hydration\'s EVM, per log: a Gamma vault\'s fee share — the ERC-20 `Transfer` of `fees / feeDivisor` of the LP fees its positions earned, from a vault the chain announced, to the Treasury\'s EVM address — and the pool-level protocol fee (`setFeeProtocol`, referendum 403), booked WHERE IT ACCRUES: a share of every swap fee, `gross fee / feeProtocol` as of that swap, attributed to the swapper who paid it. The vault share is a realization; the pool share is not, and the two are separable by `feeDestination`-independent means — the accrued rows carry `dest=\'accrued\'`. `CollectProtocol` is deliberately NOT booked: it moves fee that this stream already recognised at accrual, so counting it would double it. That makes the stream independent of whether governance has got round to collecting — on Hydration the factory owner is the Aave manager account, so collection needs a referendum and lags accrual indefinitely. The rest of a swap fee stays with the pool\'s liquidity providers and is not revenue.',
