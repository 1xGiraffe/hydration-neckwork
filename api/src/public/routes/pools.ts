...
export const poolsRoutes: FastifyPluginAsync<{ client: ClickHouseClient }> = async (fastify, opts) => {
  ...
  app.get('/v1/pools/uniswapv3/volumes', {
    schema: {
      ...
      description: [
        'Per-pool traded volume over a rolling window for the Uniswap v3 pools on Hydration\'s EVM, keyed by the pool CONTRACT address. `?pools=` filters to a subset; omitted, every pool that traded in the window is returned.',
        '`feeUsd` is the GROSS swap fee the trades paid (amount in × fee tier), valued at event time — it is what the pool charged, not what its liquidity providers keep. Where `setFeeProtocol` is on (referendum 403 enabled it on 2026-09-09; the aDOT/HOLLAR pool keeps 1/4 per token), the pool retains that share for the protocol and the LPs earn the rest. The protocol\'s share is booked as protocol revenue where it accrues, on the swap itself — see the `uniswap_v3_fee` stream on /api/v1/fees/charts — so it appears there whether or not it has ever been collected, and `feeUsd` here stays the GROSS figure that contains it.',
