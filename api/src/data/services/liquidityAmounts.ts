...
// alone), an XYK pool creation only its initial shares, so those rows arrive
// amountless and the real figure is the pool↔who transfer leg beside the event.
// One primary-key read of the page's legs from transfer_activity_by_time
// ((block_height, event_index) is its ORDER BY; ≤ the page's distinct blocks), in
// the rows' own assets only, then the explorer's own matcher
// (services/liquidityLegs.ts) pairs them by dispatch scope and adjacency — an XYK
// add, removal or pool creation takes BOTH legs against one pool account, into
// `amount` (assetA's) and `amount_b` (assetB's). A row whose leg is not there
// stays amountless: null on the wire, never a guess.
