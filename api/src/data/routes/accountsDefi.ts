...
const zLiquidityItem = z.object({
  ...
  assetId: zAssetId,
  amount: z.string().nullable().describe('Raw units of `assetId` the account moved: the event\'s own figure where it states one in that asset (an Omnipool add\'s `amount`, a stableswap add\'s or removal\'s `shares`, a claim\'s `claimed`), else — an Omnipool removal, an XYK add, removal or pool creation, whose events name shares only — the pool↔account transfer leg beside the event in the same extrinsic (or, for a runtime-hook removal, the same block), as the Explorer renders it. Null when no such leg was found; never a share count in another denomination.'),
  amountA: z.string().nullable().describe('The event\'s own `amountA` (XYK.LiquidityAdded); null elsewhere.'),
  amountB: z.string().nullable().describe('The `assetB` side, raw units: token1 of a concentrated-liquidity act, or the second leg of an XYK add, removal or pool creation — both of the pair\'s legs are recovered together from the transfers against one pool account, assetA\'s into `amount` (a creation\'s two seed deposits, as the Explorer shows them). Null when the pair was not recovered and for the other pallet events, which carry at most one amount.'),
...
export const accountsDefiRoutes: FastifyPluginAsync<{ client: ClickHouseClient }> = async (fastify, opts) => {
  ...
  app.get('/v1/accounts/:address/liquidity', {
    schema: {
      ...
      description: [
        'Omnipool/Stableswap/XYK liquidity adds and removals plus liquidity-mining reward claims, account-first. `assetRefs` names every asset the event references (both pool sides included). An event that names only shares (an Omnipool removal, an XYK add, removal or pool creation) carries the asset amounts the account actually moved, recovered from the pool↔account transfers beside it: `amount` is `assetId`\'s leg and, for XYK, `amountB` is `assetB`\'s. ' + UNSEEN_IS_EMPTY,
