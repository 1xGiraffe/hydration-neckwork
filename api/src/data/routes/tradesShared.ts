...
export const zSwapFill = z.object({
  ...
  swapper: zAccountRef.nullable().describe('The account the swap was made FOR; null when the chain reported a placeholder (an XCM-originated swap with no local origin). On an `otc` fill this is the TAKER — the account that hit the resting order, and whose direction `inputs`/`outputs` describe. The chain does not report this reliably for OTC (its Broadcast event names the order owner on most fills), so it is resolved from the OTC pallet fill event beside it.'),
  counterparty: zAccountRef.nullable().describe("An `otc` fill's MAKER: the order's owner, who took the mirror of `inputs`/`outputs` — it received the inputs and gave up the outputs. Null on every other venue, where the counterparty is a pool rather than an account."),
