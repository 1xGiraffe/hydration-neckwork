  ...
  app.get('/v1/pools/:venue/:poolKey/trades', {
    schema: {
      ...
      description: [
        ...
        'A `uniswapv3` fill is either a Router-routed hop (its Broadcast fill, netted with its siblings by `opKey`) or a direct EVM swap against the pool, booked from the pool\'s own Swap log with `opKey` `evm:<block>:<event>` and the swap\'s recipient as `swapper`; the direct ones reach this feed a few minutes behind the head (the uniswap_v3_legs derivation). Its fee leg is the GROSS pool fee the swap paid (amount in × fee tier) — what the pool charged, not what its liquidity providers keep: where `setFeeProtocol` is on, the pool retains a share of it for the protocol (1/4 per token on the aDOT/HOLLAR pool since referendum 403) and that share becomes revenue only when it is collected.',
