...
import { calcChange, downsample } from '../src/services/marketStatsService.ts'
...
describe('calcChange', () => {
  ...
  it('accepts numeric values returned by ClickHouse JSONEachRow', () => {
    expect(calcChange(1.5, 1)).toBeCloseTo(0.5, 10)
  })

  it('returns null for non-finite or malformed prices', () => {
    expect(calcChange('', '1')).toBeNull()
    expect(calcChange('not-a-price', '1')).toBeNull()
    expect(calcChange('1', 'Infinity')).toBeNull()
    expect(calcChange('1foo', '1')).toBeNull()
  })
})
...
describe('downsample', () => {
  ...
  it('always includes the last element (most recent close)', () => {
    ...
    expect(result.at(-1)).toBe(12)
  })

  it('returns exact input when length equals target', () => {
    expect(downsample([1, 2, 3, 4], 4)).toEqual([1, 2, 3, 4])
  })

  it('does not drop nearly half the data when input barely exceeds the target', () => {
    const input = Array.from({ length: 43 }, (_, i) => i)
    const result = downsample(input, 42)
    expect(result).toHaveLength(42)
    expect(result[0]).toBe(0)
    expect(result.at(-1)).toBe(42)
  })

  it('handles zero and one-point targets explicitly', () => {
