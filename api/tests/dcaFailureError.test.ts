...
describe('DCA failure errors come from the read model', () => {
  ...
  it('takes the schedule page\'s failure errors off the rows it already listed', () => {
    ...
    // stays a real absence rather than another literal that can never match. Two of
    // the fourteen only look like per-row lookups: the pool-swaps venue check, which
    // confirms the pool id an event carries over the rows one page already read —
    // bounded by that page — and the daily histogram's token arm for the
    // concentrated-liquidity rows (v3HistogramTokenArm), which resolves them to
    // their pool through the venue's own few-thousand-row projection under the
    // chart's 90-day bound. One subquery per page or per chart, not per row.
