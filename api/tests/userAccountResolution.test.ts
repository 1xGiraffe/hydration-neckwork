...
import { initUserLibraryService, loadUserLibraries, createLibrary, createTag, setTagMembers, invitesFor } from '../src/services/userLibraryService.ts'
...
describe('setTagMembers resolves through resolveDisplayAccountId', () => {
  ...
  it('is identity for a plain substrate id with no bindings loaded (existing flows stay unaffected)', async () => {
    ...
    const updated = await setTagMembers(OWNER, lib.libraryId, tag.tagId, [PLAIN_SUBSTRATE], [])
  ...
  it('stores a bound-EVM H160 member under its resolved substrate accountId, not the raw truncated alias', async () => {
    ...
    const lib = await createLibrary(OWNER, 'L', '', 'private')
    const tag = await createTag(OWNER, lib.libraryId, { name: 'T' })
    const updated = await setTagMembers(OWNER, lib.libraryId, tag.tagId, [BOUND_EVM_H160], [])
  ...
  it('removes by the same resolved id it was added under', async () => {
    ...
    const lib = await createLibrary(OWNER, 'L', '', 'private')
    const tag = await createTag(OWNER, lib.libraryId, { name: 'T' })
    await setTagMembers(OWNER, lib.libraryId, tag.tagId, [BOUND_EVM_H160], [])
    const updated = await setTagMembers(OWNER, lib.libraryId, tag.tagId, [], [BOUND_EVM_H160])
    expect([...updated.members]).toHaveLength(0)
  })
})

describe('/user/libraries/:id/invites resolves the grantee through resolveDisplayAccountId', () => {
  ...
  async function login(f: Awaited<ReturnType<typeof build>>, w = wallet()) {
    ...
    return v.json() as { token: string; me: { account: { accountId: string }; libraries: { libraryId: string }[] } }
  }
  ...
  it('an invite addressed to a bound-EVM H160 lands under the accountId the invitee actually logs in as', async () => {
    ...
    const libId = owner.me.libraries[0].libraryId
    const invite = await f.inject({
      method: 'POST', url: `/user/libraries/${libId}/invites`,
    ...
    expect(invitesFor(BOUND_SUBSTRATE).map(l => l.libraryId)).toEqual([libId])
