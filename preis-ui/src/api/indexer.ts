...
// recent the newest indexed block is. Hydration produces a block every ~6s and the
// price pipeline trails raw ingestion by a handful of blocks, so a two-minute window
// covers normal operation while a stall — whose block timestamp stops advancing —
// goes amber. blocksBehindHead cannot carry this on its own: without a chain-head
// sample the API measures it against raw ingestion's own head, and both pipelines
// stall together, so it reads 0 exactly when the indicator matters most.
