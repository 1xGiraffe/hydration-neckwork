    ...
    <Dialog.Root open={isOpen} onOpenChange={(open) => { if (!open) onClose() }}>
      <Dialog.Portal>
        <style>{`
          ...
          .picker-visually-hidden { position: absolute; width: 1px; height: 1px; padding: 0; margin: -1px; overflow: hidden; clip: rect(0, 0, 0, 0); white-space: nowrap; border: 0; }
          ...
          @media (max-width: 768px) {
            .picker-table-head { grid-template-columns: minmax(0, 1fr) 80px 58px 80px; gap: 8px; padding: 10px 14px; }
            .picker-table-head .col-1h, .picker-table-head .col-7d { display: none; }
            .picker-row { grid-template-columns: minmax(0, 1fr) 80px 58px 80px; gap: 8px; padding: 11px 14px; }
            .picker-row .col-1h, .picker-row .col-7d { display: none; }
            .picker-foot { flex-direction: column; align-items: flex-start; gap: 6px; }
          }
          @media (max-width: 340px) {
            .picker-table-head { grid-template-columns: minmax(0, 1fr) 70px 52px 80px; gap: 8px; padding: 10px 12px; }
            .picker-row { grid-template-columns: minmax(0, 1fr) 70px 52px 80px; gap: 8px; padding: 11px 12px; }
          }
        `}</style>
        ...
        >
          <Dialog.Title className="picker-visually-hidden">Select trading pair</Dialog.Title>
          <Dialog.Description className="picker-visually-hidden">
            Search for an asset or pair, then choose one to display its price chart.
          </Dialog.Description>
          ...
          <div ref={listRef} id={listboxId} className="picker-table" role="listbox" aria-label="Trading pairs">
            {rows.length === 0 ? (
              <div style={{
